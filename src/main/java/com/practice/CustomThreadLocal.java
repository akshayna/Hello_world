...
public class CustomThreadLocal<T> {
	 ...
	 * invoked the {@link #set} method, in which case the <tt>initialValue</tt>
	 ...
	 *
	 * <p>
	 * This implementation simply returns <tt>null</tt>; if the programmer
	 * desires thread-local variables to have an initial value other than
	 * <tt>null</tt>, <tt>ThreadLocal</tt> must be subclassed, and this method
	 ...
	 */
	public T get() {
		CustomThread t = CustomThread.currentThread();
		CustomThreadLocalMap map = getMap(t);
		if (map != null) {
			CustomThreadLocalMap.CustomEntry e = map.getEntry(this);
			if (e != null)
				return (T) e.value;
		}
	 ...
	 * invocations of the <tt>initialValue</tt> method in the current thread.
	 *
	 * @since 1.5
	 */
	public void remove() {
		CustomThreadLocalMap m = getMap(CustomThread.currentThread());
		if (m != null)
			m.remove(this);
	}
	 ...
	 * @return the map
	 */
	CustomThreadLocalMap getMap(CustomThread t) {
	 ...
	 *            the map to store.
	 */
	void createMap(CustomThread t, T firstValue) {
		t.threadLocals = new CustomThreadLocalMap(this, firstValue);
	}
	 ...
	 */
	static CustomThreadLocalMap createInheritedMap(CustomThreadLocalMap parentMap) {
		return new CustomThreadLocalMap(parentMap);
	}
	 ...
	 */
	static class CustomThreadLocalMap {
		 ...
		 */
		static class CustomEntry extends WeakReference<CustomThreadLocal> {
			/** The value associated with this ThreadLocal. */
			Object value;

			CustomEntry(CustomThreadLocal k, Object v) {
				super(k);
				value = v;
			}
		}
		 ...
		 */
		CustomThreadLocalMap(CustomThreadLocal firstKey, Object firstValue) {
			table = new CustomEntry[INITIAL_CAPACITY];
			int i = firstKey.threadLocalHashCode & (INITIAL_CAPACITY - 1);
			table[i] = new CustomEntry(firstKey, firstValue);
			size = 1;
		 ...
		 *            the map associated with parent thread.
		 */
		private CustomThreadLocalMap(CustomThreadLocalMap parentMap) {
			CustomEntry[] parentTable = parentMap.table;
			int len = parentTable.length;
			setThreshold(len);
			table = new CustomEntry[len];

			for (int j = 0; j < len; j++) {
				CustomEntry e = parentTable[j];
				if (e != null) {
					CustomThreadLocal key = e.get();
					if (key != null) {
						Object value = key.childValue(e.value);
						CustomEntry c = new CustomEntry(key, value);
		 ...
		 */
		private CustomEntry getEntry(CustomThreadLocal key) {
			int i = key.threadLocalHashCode & (table.length - 1);
			CustomEntry e = table[i];
			if (e != null && e.get() == key)
				return e;
			else
		 ...
		 * @return the entry associated with key, or null if no such
		 */
		private CustomEntry getEntryAfterMiss(CustomThreadLocal key, int i, CustomEntry e) {
			CustomEntry[] tab = table;
			int len = tab.length;

			while (e != null) {
				CustomThreadLocal k = e.get();
				if (k == key)
					return e;
				if (k == null)
					expungeStaleEntry(i);
		 ...
		 *            the value to be set
		 */
		private void set(CustomThreadLocal key, Object value) {
			...
			for (CustomEntry e = tab[i]; e != null; e = tab[i = nextIndex(i, len)]) {
				CustomThreadLocal k = e.get();
			...
			tab[i] = new CustomEntry(key, value);
			int sz = ++size;
		 ...
		 */
		private void remove(CustomThreadLocal key) {
			CustomEntry[] tab = table;
			int len = tab.length;
			int i = key.threadLocalHashCode & (len - 1);
			for (CustomEntry e = tab[i]; e != null; e = tab[i = nextIndex(i, len)]) {
		 ...
		 *            for key.
		 */
		private void replaceStaleEntry(CustomThreadLocal key, Object value, int staleSlot) {
			CustomEntry[] tab = table;
			int len = tab.length;
			CustomEntry e;
			...
			tab[staleSlot] = new CustomEntry(key, value);
		...
		private int expungeStaleEntry(int staleSlot) {
			...
			// expunge CustomEntry at staleSlot
			tab[staleSlot].value = null;
			tab[staleSlot] = null;
			size--;
		 ...
		 *
		 * @param n
		 *            scan control: <tt>log2(n)</tt> cells are scanned, unless a
		 *            stale entry is found, in which case
		 *            <tt>log2(table.length)-1</tt> additional cells are
		 *            scanned. When called from insertions, this parameter is
		 *            the number of elements, but when from replaceStaleEntry,
		 *            it is the table length. (Note: all this could be changed
		 *            to be either more or less aggressive by weighting n
		 *            instead of just using straight log n. But this version is
		 ...
		 */
		private void resize() {
			CustomEntry[] oldTab = table;
			int oldLen = oldTab.length;
			int newLen = oldLen * 2;
			CustomEntry[] newTab = new CustomEntry[newLen];
			int count = 0;

			for (int j = 0; j < oldLen; ++j) {
				CustomEntry e = oldTab[j];
				if (e != null) {
					CustomThreadLocal k = e.get();
					if (k == null) {
