...
import com.practice.CustomThreadLocal.CustomThreadLocalMap;
...
public class CustomThread implements Runnable {
	...
	private CustomThreadGroup group;
	 ...
	 */
	CustomThreadLocalMap threadLocals = null;

	/*
	 * InheritableThreadLocal values pertaining to this thread. This map is
	 * maintained by the InheritableThreadLocal class.
	 */
	CustomThreadLocal.CustomThreadLocalMap inheritableThreadLocals = null;
	 ...
	 */
	public static native CustomThread currentThread();
	 ...
	 */
	private void init(CustomThreadGroup g, Runnable target, String name, long stackSize) {
	 ...
	 */
	private void init(CustomThreadGroup g, Runnable target, String name, long stackSize, AccessControlContext acc) {
		if (name == null) {
		...
		CustomThread parent = currentThread();
		CustomSecurityManager security = CustomSystem.getSecurityManager();
		if (g == null) {
		...
		if (parent.inheritableThreadLocals != null)
			this.inheritableThreadLocals = CustomThreadLocal.createInheritedMap(parent.inheritableThreadLocals);
	 ...
	 */
	CustomThread(Runnable target, AccessControlContext acc) {
	 ...
	 *             thread group
	 */
	public CustomThread(CustomThreadGroup group, Runnable target) {
	 ...
	 *             thread group
	 */
	public CustomThread(CustomThreadGroup group, String name) {
	 ...
	 *             methods.
	 */
	public CustomThread(CustomThreadGroup group, Runnable target, String name) {
	 ...
	 * @since 1.4
	 */
	public CustomThread(CustomThreadGroup group, Runnable target, String name, long stackSize) {
	 ...
	 * <p>
	 * If this thread is different from the current thread (that is, the current
	 * thread is trying to stop a thread other than itself) or <code>obj</code>
	 * is not an instance of <code>ThreadDeath</code>, the security manager's
	 ...
	 * <p>
	 * The thread represented by this thread is forced to stop whatever it is
	 * doing abnormally and to throw the <code>Throwable</code> object
	 * <code>obj</code> as an exception. This is an unusual action to take;
	 * normally, the <code>stop</code> method that takes no arguments should be
	 * used.
	 * <p>
	 * It is permitted to stop a thread that has not yet been started. If the
	 * thread is eventually started, it immediately terminates.
	 ...
	 * @see SecurityManager#checkAccess(Thread)
	 * @see SecurityManager#checkPermission
	 * @deprecated This method is inherently unsafe. See {@link #stop()} for
	 *             details. An additional danger of this method is that it may
	 *             be used to generate exceptions that the target thread is
	 *             unprepared to handle (including checked exceptions that the
	 *             thread could not possibly throw, were it not for this
	...
	public final synchronized void stop(Throwable obj) {
		...
		if (security != null) {
			checkAccess();
			if ((this != CustomThread.currentThread()) || (!(obj instanceof ThreadDeath))) {
	...
	public void interrupt() {
		if (this != CustomThread.currentThread())
			checkAccess();
	 ...
	 */
	public final CustomThreadGroup getThreadGroup() {
		return group;
	}
	 ...
	 */
	public static int enumerate(CustomThread tarray[]) {
	...
	public String toString() {
		CustomThreadGroup group = getThreadGroup();
		if (group != null) {
	...
	public StackTraceElement[] getStackTrace() {
		if (this != CustomThread.currentThread()) {
			...
			StackTraceElement[][] stackTraceArray = dumpThreads(new CustomThread[] { this });
	 ...
	 * @since 1.5
	 */
	public static Map<CustomThread, StackTraceElement[]> getAllStackTraces() {
		// check for getStackTrace permission
		SecurityManager security = System.getSecurityManager();
		if (security != null) {
			security.checkPermission(SecurityConstants.GET_STACK_TRACE_PERMISSION);
		...
		CustomThread[] threads = getThreads();
		StackTraceElement[][] traces = dumpThreads(threads);
		Map<CustomThread, StackTraceElement[]> m = new HashMap<>(threads.length);
	...
	private native static StackTraceElement[][] dumpThreads(CustomThread[] customThreads);

	private native static CustomThread[] getThreads();
	...
	public interface UncaughtExceptionHandler {
		 ...
		 *            the exception
		 */
		void uncaughtException(CustomThread customThread, Throwable e);
	}
	...
	public static void setDefaultUncaughtExceptionHandler(UncaughtExceptionHandler eh) {
		CustomSecurityManager sm = CustomSystem.getSecurityManager();
		if (sm != null) {
